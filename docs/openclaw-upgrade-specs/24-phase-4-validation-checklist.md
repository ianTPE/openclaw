...
`openclaw memory search` 是 CtxFST retrieval 的正式 CLI 入口，可直接用於驗收 graph expansion：

```bash
openclaw memory search --expand-graph "What is required before Analyze Resume"
openclaw memory search --expand-graph "Analyze Resume prerequisites"
openclaw memory search --expand-graph "What happens after Resume Parsed"
openclaw memory search --expand-graph "API"
```

更細的 relation filter、max-expanded-entities 之類控制，目前仍以 integration tests 覆蓋為主。
